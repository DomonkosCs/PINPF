...
"""
Full baseline comparison for the nonlinear model on the test split.
...
ground truth.  Prints LaTeX and markdown tables.
"""
...
from problems.configs import ConfigNonlinear
...
from flow import create_euler_adaptive
from utils import (
...
def _make_task(config, i):
    ...
    meas_sub = config.meas_model_const(
        alpha=config.alpha[i].unsqueeze(0).to(DEVICE),
        sigma=config.sigma[i].unsqueeze(0).to(DEVICE),
    )
...
def save_summary(results_dict, n_tasks, out_dir):
    """Save combined summary (.txt, .tex, .pt) after all methods are done."""
    ...
    with open(tex_path, "w") as f:
        ...
        f.write(r"    \label{tab:nonlinear_comparison}" + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Full baseline comparison for nonlinear model on test split."
    )
    parser.add_argument(
        "--dim",
        type=int,
        default=15,
        help="State dimension (derives data/GT paths if not set)",
    )
    ...
    parser.add_argument("--d-lambda", type=float, default=0.5)
    parser.add_argument("--n-particles", type=int, default=2000)
    parser.add_argument("--n-particles-svgd", type=int, default=500)
    parser.add_argument("--n-particles-annealed", type=int, default=1000)
    parser.add_argument(
        "--gt-path",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--gt-samples", type=int, default=10000, help="Subsample GT (0 = use all)"
    )
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--svgd-iter", type=int, default=50)
    parser.add_argument("--svgd-lr", type=float, default=0.2)
    parser.add_argument("--annealed-steps", type=int, default=10)
    parser.add_argument("--annealed-mcmc-per-step", type=int, default=2)
    parser.add_argument("--annealed-step-size", type=float, default=0.1)
    ...
    # Derive paths from --dim if not explicitly set
    if args.data_path is None:
        args.data_path = f"data/nonlinear_{args.dim}d/dataset.pt"
    if args.gt_path is None:
        args.gt_path = f"data/nonlinear_{args.dim}d/gt_{args.mode}.pt"
    ...
    if args.train_config:
        ...
        args.layers = 6
        args.hidden_dim = 64
        args.features = "grad_log_p,log_h,grad_log_h"
    ...
    x_gt = gt_data["samples"]
    if args.gt_samples > 0 and x_gt.shape[1] > args.gt_samples:
        x_gt = x_gt[:, : args.gt_samples, :]
    x_gt = x_gt.to(DEVICE)
    print(f"Ground truth: {x_gt.shape}")
    ...
    for i in range(n_tasks):
        ...
        x_dhf, t = _timed(_run_dhf)
        ed, swd = _metrics(x_dhf)
