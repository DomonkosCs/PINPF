"""
Full baseline comparison for the GMM model on the test split.

Evaluates PINPF, Incompressible Flow, NSF (optional), SVGD, and Annealed MCMC
against the analytic GMM posterior. Prints LaTeX and markdown tables.
"""
...
from problems.configs import ConfigGMM
from models import NeuralFlowModel, load_neural_flow_model
from flow import NeuralFlow, IncompressibleFlow, create_euler_adaptive
from utils import (
...
def _make_task(config, i):
    ...
    meas_sub = config.meas_model_const(
        weights=config.gmm_weights[i].unsqueeze(0).to(DEVICE),
        sigmas=config.gmm_sigmas[i].unsqueeze(0).to(DEVICE),
    )
    z_task = config.zs[i].unsqueeze(0).to(DEVICE)
    return prior_sub, meas_sub, z_task
...
def save_summary(results_dict, n_tasks, out_dir):
    ...
    with open(txt_path, "w") as f:
        header = f"{'Method':<30} {'ED':>18} {'SWD':>18} {'Time/task':>10}"
        f.write(header + "\n")
        f.write("-" * len(header) + "\n")
        for name, res in results_dict.items():
    ...
    with open(tex_path, "w") as f:
        ...
        f.write(r"    \label{tab:gmm_comparison}" + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Full baseline comparison for GMM model on test split."
    )
    ...
    # Load PINPF model
    print(f"Loading PINPF model from {args.model_path}...")
    ...
    # Load NSF model — read architecture from checkpoint to match training exactly
    print(f"Loading NSF model from {args.nsf_path}...")
    nsf_ckpt = torch.load(args.nsf_path, map_location=DEVICE, weights_only=False)
    nsf_state_dim = nsf_ckpt["state_dim"]
    nsf_context_dim = nsf_ckpt["context_dim"]
    nsf_train_args = nsf_ckpt["args"]
    nsf_model = build_conditional_nsf(
        state_dim=nsf_state_dim,
        context_dim=nsf_context_dim,
        n_blocks=nsf_train_args.n_blocks,
        hidden_features=nsf_train_args.hidden,
        num_bins=nsf_train_args.bins,
        dropout=nsf_train_args.dropout,
    ).to(DEVICE)
    nsf_model.load_state_dict(nsf_ckpt["flow_state"])
    nsf_model.eval()
    print(f"NSF: state_dim={nsf_state_dim}, context_dim={nsf_context_dim}, "
          f"n_blocks={nsf_train_args.n_blocks}, hidden={nsf_train_args.hidden}")
    ...
    algos = ["PINPF", "Incompressible Flow", "NSF", "SVGD", "Annealed MCMC"]
    ...
    for i in range(n_tasks):
        print(f"\nTask {i + 1}/{n_tasks}")
        prior_sub, meas_sub, z_task = _make_task(config, i)

        # Analytic GT for this task
        analytic_i = AnalyticGMMPosterior(prior_sub, meas_sub, z_task)
        gt_i = analytic_i.sample(args.gt_samples).squeeze(0).to(DEVICE)  # (N, D)
        ...
        # ── NSF ──
        b_mu = config.prior_model.mu_prior[i : i + 1]
        b_var = config.prior_model.var_prior[i : i + 1]
        b_z = config.zs[i : i + 1]
        b_w = config.gmm_weights[i : i + 1]
        b_sig = config.gmm_sigmas[i : i + 1]
        context = make_conditioning(b_mu, b_var, b_z, b_w, b_sig).to(DEVICE)

        def _run_nsf():
            with torch.no_grad():
                return nsf_model.sample(args.n_particles_nsf, context=context).squeeze(0)

        x_nsf, t = _timed(_run_nsf)
        ed, swd = _metrics(x_nsf)
        metrics["NSF"]["ed"].append(ed)
        metrics["NSF"]["swd"].append(swd)
        metrics["NSF"]["time"].append(t)
        print(f"  NSF:     ED={ed:.4f}, SWD={swd:.4f}, t={t:.4f}s")
    ...
    # ── Summary ──
    print("\n" + "=" * 75)
    print(f"{'Method':<30} {'ED':>18} {'SWD':>18} {'Time/task':>10}")
    print("-" * 75)
